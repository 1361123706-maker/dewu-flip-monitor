...
def normalize_trend(value):
    if value is None:
        return None
    mapping = {"rising":"上涨", "rise":"上涨", "up":"上涨", "上涨":"上涨", "falling":"下跌", "fall":"下跌", "down":"下跌", "下跌":"下跌", "stable":"稳定", "flat":"稳定", "稳定":"稳定", "平稳":"稳定"}
...
def get_buy_price(item):
    for key in ("best_effective_buy_price", "effective_buy_price", "final_buy_price", "estimated_final_price", "buy_price", "sku_buy_price"):
...
def get_sku_context(item):
    ...
    effective = to_float(item.get("best_effective_buy_price")) or to_float(item.get("effective_buy_price"))
    ...
    if size_prices:
        ...
        for size, price in size_prices.items():
            # 活动引擎明确说明的 base_price 与该尺码原价一致时，
            # 才将活动后价格用于这个 SKU；避免把商品级优惠误套到其他尺码。
            if effective is not None and base_price is not None and abs(base_price - price) < 0.01:
                sku_price = effective
            elif effective is not None and current_size == size and base_price is None:
                # 没有 base_price 证据时，只对采集器明确选中的当前尺码使用。
                sku_price = effective
            else:
                sku_price = price
            result.append((size, sku_price))
        return result

    buy_price = effective or get_buy_price(item)
    return [(None, buy_price)] if buy_price is not None else []


def get_dewu_price(item):
    for key, label, kind in (("dewu_channel_price", "识货：得物渠道售价", "channel_price"), ("trend_current_price", "识货：当前同款同规格价格", "current_price"), ("dewu_price", "识货：得物售价", "dewu_price")):
        value = to_float(item.get(key))
        if value is not None and value > 0:
...
def price_evidence(item):
    current = to_float(item.get("trend_current_price")) or to_float(item.get("dewu_channel_price")) or to_float(item.get("dewu_price"))
    status = "confirmed_current_price" if to_float(item.get("trend_current_price")) else ("channel_price_only" if to_float(item.get("dewu_channel_price")) else ("dewu_price_only" if current else "no_dewu_price"))
    ...
    return {"current": current, "low": low, "high": high, "position": position, "downside": downside, "status": status}
...
def turnover(item):
    ...
    if sales_7d is not None and sales_7d > 0:
        ...
        confidence = "unknown"
    return {"sales_7d": sales_7d, "sales_30d": sales_30d, "velocity": velocity, "evidence": evidence, "confidence": confidence or "unknown", "days": None}
...
def analyze(item, sku_size=None, sku_buy_price=None):
    ...
    if dewu_price is None:
        return {"name": name, "product_code": normalize_text(item.get("product_code")), "color": normalize_text(item.get("color")), "sku_size": sku_size, "sku_buy_price": money(buy_price), "grade": "D", "category": item.get("category"), "buy_price": money(buy_price), "dewu_price": None, "hard_risks": ["没有有效得物价格"], "reason": "没有有效得物价格", "capital": CAPITAL, "profit_type": "estimated", "profit_status": "estimated_not_ready", "profit_estimate_ready": False}
    ...
    return {
        ...
        "sku_identity_complete": bool(item.get("product_code") and item.get("color") and (sku_size is not None or not get_size_price_map(item))),
...
def main():
    ...
    candidates = [x for x in results if x.get("grade") in {"A", "B"} and (x.get("estimated_profit") or 0) >= MIN_PROFIT and (x.get("profit_rate") or 0) >= MIN_PROFIT_RATE]
    candidates.sort(key=lambda x: (0 if x.get("grade") == "A" else 1, -(x.get("estimated_profit") or 0), -(x.get("profit_rate") or 0)))
    summary = {"total": len(results), "A": sum(x.get("grade") == "A" for x in results), "B": sum(x.get("grade") == "B" for x in results), "C": sum(x.get("grade") == "C" for x in results), "D": sum(x.get("grade") == "D" for x in results), "estimated_profit_count": sum(x.get("profit_status") == "estimated_ready" for x in results)}
    output = {
        "updated_at": datetime.now(timezone.utc).isoformat(), "capital": CAPITAL,
        "rules": {"max_days": MAX_DAYS, "min_profit": MIN_PROFIT, "min_profit_rate": MIN_PROFIT_RATE, "max_downside_loss": MAX_DOWNSIDE_LOSS, "dewu_net_rate": DEWU_NET_RATE, "default_buy_shipping": DEFAULT_BUY_SHIPPING, "profit_type": "estimated", "profit_status": "estimated_ready", "profit_formula": "得物售价×92%-买入价-6元", "sku_level": True, "coupon_aware": True, "multi_platform_buy_sources": True},
        "summary": summary, "monitored_count": len(results), "candidate_count": len(candidates), "grade_a_count": summary["A"], "grade_b_count": summary["B"], "grade_c_count": summary["C"], "grade_d_count": summary["D"], "candidates": candidates, "results": results
    }
