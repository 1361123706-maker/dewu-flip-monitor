...
def extract_coupon_offers(text, links):
    ...
    def stackable_near(position):
        ...
        return any(marker in window for marker in (
            "可叠加", "叠加使用", "可同时使用", "同时使用", "一起使用", "可以叠加",
        ))

    coupon_links = [x for x in links if x.get("is_coupon")]

    full_reduce = re.compile(
        r"满\s*([0-9]+(?:\.[0-9]+)?)\s*元?"
        r"[^。；;\n]{0,25}?"
        r"(?:减|立减|优惠)\s*([0-9]+(?:\.[0-9]+)?)\s*元?",
        re.I,
    )
    for match in full_reduce.finditer(text):
        ...
        offers.append({
            ...
            "threshold": threshold,
            "url": None,
            "stackable": stackable_near(match.start()),
    ...
    for offer_type, pattern in coupon_patterns:
        for match in re.finditer(pattern, text, re.I):
            ...
            platform = link.get("platform", "") if link else ""
            offers.append({
                "type": offer_type,
                "platform": platform,
                "amount": amount,
                ...
                "stackable": any(marker in nearby for marker in (
                    "可叠加", "叠加使用", "可同时使用", "同时使用", "一起使用",
                )),
                "evidence": clean_text(nearby),
                "source": "shihuo_detail_text",
            })
    ...
    # 领取入口只用于展示/通知，不参与金额计算。
    for link in coupon_links:
        anchor = clean_text(link.get("anchor_text") or link.get("text")) or "优惠领取入口"
        offers.append({
            "type": "优惠领取入口",
    ...
    for offer in offers:
        key = (
            offer.get("type"), offer.get("platform"), offer.get("amount"),
            offer.get("threshold"), offer.get("url"), offer.get("evidence"),
        )
        if key in seen:
            continue
        seen.add(key)
